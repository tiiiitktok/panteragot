...
const CANCELLED_WORDS = ["cancel", "refund", "reembols", "chargeback", "expired", "expirad", "recus", "denied", "declined", "failed"];
...
function detectTipo(body, queryTipo) {
  if (queryTipo === "gerada" || queryTipo === "aprovada" || queryTipo === "cancelada") return queryTipo;
...
function detectExtra(body) {
  const cliente = findFirst(body, /customer_?name|buyer_?name|cliente|nome/i);
  const produto = findFirst(body, /product_?name|produto|item_?name|plano/i);
  return { cliente: cliente ? String(cliente) : null, produto: produto ? String(produto) : null };
}
