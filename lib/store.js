...
// ---------- foto de perfil (guardada como data URL base64, já reduzida no navegador) ----------
async function getAvatarForUser(userId) {
  return getValue(`avatar:${userId}`, null);
}
async function setAvatarForUser(userId, dataUrl) {
  return setValue(`avatar:${userId}`, dataUrl);
}
async function deleteAvatarForUser(userId) {
  return setValue(`avatar:${userId}`, null);
}
